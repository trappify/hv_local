...
def test_sample_when_full_keeps_previous_until_full() -> None:
    assert (
        sample_when_full(
            current_value=10.0,
            soc=50.0,
...
def test_sample_total_when_full_requires_all_modules_full() -> None:
    ...
    assert (
        sample_total_when_full(modules=modules, threshold=99.5, previous_value=None)
        == 11.0
    )
    ...
    assert (
        sample_total_when_full(modules=modules_not_full, threshold=99.5, previous_value=10.5)
        == 10.5
    )
...
def test_sample_total_when_full_ignores_incomplete_payloads() -> None:
    ...
    assert (
        sample_total_when_full(modules=modules, threshold=99.5, previous_value=10.0)
        == 10.0
    )

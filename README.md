...
This repository hosts an **unofficial** Home Assistant integration that talks directly to a Homevolt battery gateway over the local network. It mirrors the JSON endpoints exposed by the gateway (`/status.json`, `/ems.json`, `/schedule.json`, `/error_report.json`) and exposes the values as coordinator-driven sensors so you can track battery SOC, temperature, voltages, grid/solar/load power, EMS schedule, and health/problem signals from any Home Assistant instance.
...
>
> 🖼️ Branding note: The included icon is AI-generated and not an official Homevolt/Tibber mark.
